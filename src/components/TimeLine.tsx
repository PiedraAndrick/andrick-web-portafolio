import { mogra } from "@/lib";
import { dataAboutPage } from "../../dataIndex";
                    ...
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                            ...
                            <div className="flex flex-col sm:flex-row items-start mb-1 
                                ...
                                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-blue-300 bg-blue-400/20 rounded-full">{data.date}</time>
                                <div className="text-xl font-bold text-white">{data.subtitle}</div>
                            </div>
                            <div className={`text-cyan-300/90 tex ${mogra.className}`}>{data.description}</div>
                        </div>
                    ))}
                </div>
            </div>
